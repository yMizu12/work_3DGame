...
struct DISSOLVE
{
	int		Enable;		// 有効にするかのフラグ
	float	threshold;	// しきい値（適用するライン）2値化の時とか
	int		fill[2];	// 16バイト刻みにするように（アライメント
...
Texture2D		g_Fractalnoise : register( t1 );		// ディゾルブ用テクスチャ
						 ...
						 out float4 outDiffuse		: SV_Target )
{
	...
	//======================================================================
	{
		...
		if (dissolve.Enable == 1)
		{
			...
			//r指定、ｒを取り出してる（フラクタルがモノトーンだから何を取り出しても同じ値
			if (noise.r < dissolve.threshold)	// しきい値より小さい場合
			{
				outDiffuse.a = 0.0f;		// α値を０に（透明する
			}
			else if (noise.r < dissolve.threshold + 0.2f)	// スレッシュホールドより大きく（前の分岐）、それよりも0.1大きい
			{
				float temp = (noise.r - dissolve.threshold) * 5.0f;//10.0f;//0-1.0が入る
