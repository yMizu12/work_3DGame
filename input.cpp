...
HRESULT InitKeyboard(HINSTANCE hInst, HWND hWnd)
{
	...
	{
		MessageBox(hWnd, "キーボードなし！", "警告！", MB_ICONWARNING);
		return hr;
	}
...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//* y-axis (forward)
		if (dijs.lY < 0) { padState[i] |= STICK_UP_L; PrintDebugProc("BUTTON_UP\n"); }
		//* y-axis (backward)
		if (dijs.lY > 0) { padState[i] |= STICK_DOWN_L; PrintDebugProc("BUTTON_DOWN\n"); }
		//* x-axis (left)
		if (dijs.lX < 0 ){ padState[i] |= STICK_LEFT_L; PrintDebugProc("BUTTON_LEFT\n"); }
		//* x-axis (right)
		if (dijs.lX > 0) { padState[i] |= STICK_RIGHT_L; PrintDebugProc("BUTTON_RIGHT\n"); }


		//* y-axis (forward)
		if (dijs.lRz < 0) { padState[i] |= STICK_UP_R; PrintDebugProc("BUTTON_UP_RRR\n"); }
		//* y-axis (backward)
		if (dijs.lRz > 0) { padState[i] |= STICK_DOWN_R; PrintDebugProc("BUTTON_DOWN_RRR\n"); }
		//* x-axis (left)
		if (dijs.lZ < 0 ){ padState[i] |= STICK_LEFT_R; PrintDebugProc("BUTTON_LEFT_RRR\n"); }
		//* x-axis (right)
		if (dijs.lZ > 0) { padState[i] |= STICK_RIGHT_R; PrintDebugProc("BUTTON_RIGHT_RRR\n"); }

		//* Ｘボタン
		if (dijs.rgbButtons[rgbButtons_X] & 0x80) { padState[i] |= BUTTON_X; PrintDebugProc("BUTTON_X\n"); }
		//* Ａボタン
		if (dijs.rgbButtons[rgbButtons_A] & 0x80) { padState[i] |= BUTTON_A; PrintDebugProc("BUTTON_A\n"); }
		//* Ｂボタン
		if (dijs.rgbButtons[rgbButtons_B] & 0x80) { padState[i] |= BUTTON_B; PrintDebugProc("BUTTON_B\n"); }
		//* Ｙボタン
		if (dijs.rgbButtons[rgbButtons_Y] & 0x80) { padState[i] |= BUTTON_Y; PrintDebugProc("BUTTON_Y\n"); }
		//* Ｌボタン
		if (dijs.rgbButtons[rgbButtons_L] & 0x80) { padState[i] |= BUTTON_L; PrintDebugProc("BUTTON_L\n"); }
		//* Ｒボタン
		if (dijs.rgbButtons[rgbButtons_R] & 0x80) { padState[i] |= BUTTON_R; PrintDebugProc("BUTTON_R\n"); }
		//* Ｌ2ボタン
		if (dijs.rgbButtons[rgbButtons_L2] & 0x80) { padState[i] |= BUTTON_L2; PrintDebugProc("BUTTON_L2\n"); }
		//* Ｒ2ボタン
		if (dijs.rgbButtons[rgbButtons_R2] & 0x80) { padState[i] |= BUTTON_R2; PrintDebugProc("BUTTON_R2\n"); }
		//* Selectボタン
		if (dijs.rgbButtons[rgbButtons_SELECT] & 0x80) { padState[i] |= BUTTON_SELECT; PrintDebugProc("BUTTON_SELECT\n"); }
		//* ＳＴＡＲＴボタン
		if (dijs.rgbButtons[rgbButtons_START] & 0x80) { padState[i] |= BUTTON_START; PrintDebugProc("BUTTON_START\n"); }
