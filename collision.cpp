...
BOOL CollisionBC(XMFLOAT3 pos1, XMFLOAT3 pos2, float r1, float r2)
{
	...
	temp = XMVector3LengthSq(temp);			// 2点間の距離（2乗した物）（ベクトルの長さの２乗を計算する）
	float lenSq = 0.0f;
...
BOOL RayCast(XMFLOAT3 xp0, XMFLOAT3 xp1, XMFLOAT3 xp2, XMFLOAT3 xpos0, XMFLOAT3 xpos1, XMFLOAT3 *hit, XMFLOAT3 *normal)
{
	...
	{	// ポリゴンの外積をとって法線を求める
		...
		nor = XMVector3Normalize(nor);		// 計算しやすいように法線をノーマライズしておく(ベクトルの長さを１に)
		XMStoreFloat3(normal, nor);			// 求めた法線を入れておく
	}

	// ポリゴン平面と線分の内積とり衝突している可能性を調べる（鋭角なら＋、鈍角ならー、直角なら０）
	vec1 = pos0 - p0;
	vec2 = pos1 - p0;
	{	// 求めたポリゴンの法線と２つのベクトル（線分の両端とポリゴン上の任意の点）の内積とり衝突している可能性を調べる
	...
	{	// ポリゴンと線分の交点を求める
		...
		{	// 求めた交点がポリゴンの中にあるか調べる
			...
			// 外積で各辺の法線を求めて、ポリゴンの法線との内積をとり符号をチェックする
			XMVECTOR	n1, n2, n3;
